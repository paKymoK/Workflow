...
public class WorkflowNodeReader implements Converter<Json, ListWorkflowNode> {
  private final ObjectMapper objectMapper;

  @Override
  public ListWorkflowNode convert(@NonNull Json source) {
    final JsonNode tree = readTree(source);
    try {
      return objectMapper.convertValue(tree, ListWorkflowNode.class);
    } catch (Exception ex) {
