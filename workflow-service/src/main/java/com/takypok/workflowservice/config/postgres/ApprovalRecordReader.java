...
public class ApprovalRecordReader implements Converter<Json, ListApprovalRecord> {
  private final ObjectMapper objectMapper;

  @Override
  public ListApprovalRecord convert(@NonNull Json source) {
    final JsonNode tree = readTree(source);
    try {
      return objectMapper.convertValue(tree, ListApprovalRecord.class);
    } catch (Exception ex) {
