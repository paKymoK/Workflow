...
public class LinkedTicketReader implements Converter<Json, ListLinkedTickets> {
  private final ObjectMapper objectMapper;

  @Override
  public ListLinkedTickets convert(@NonNull Json source) {
    final JsonNode tree = readTree(source);
    try {
      return objectMapper.convertValue(tree, ListLinkedTickets.class);
    } catch (Exception ex) {
