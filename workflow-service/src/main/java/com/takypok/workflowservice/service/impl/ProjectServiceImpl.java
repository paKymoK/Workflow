...
public class ProjectServiceImpl implements ProjectService {
  ...
  @Override
  public Mono<IssueType> updateIssueType(Long id, IssueTypeUpdateRequest request) {
    return issueTypeRepository
        .findById(id)
        .switchIfEmpty(
            Mono.error(
                new ApplicationException(Message.Application.ERROR, "Issue type not existed")))
        .flatMap(
            issueType -> {
              issueType.setWorkflowId(request.getWorkflowId());
