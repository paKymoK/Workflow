...
import type { ChatMessage, ConversationSummary, PresenceStatus, ReactionSummary } from '@/src/api/chatTypes';

const wsBaseUrl = (API_BASE_URL ?? 'http://localhost:8080').replace(/^http/, 'ws');
...
/**
 * Single app-wide connection to chat-service's WebSocket, mounted once at the app root (see
 * App.tsx) so unread badges/live messages work regardless of which tab is open. Auth handshake
 * mirrors the web app's useChatSocket and chat-service's ChatWebSocketHandler: send the raw
 * access token as the first frame. RN ships a native WebSocket global, no polyfill needed.
 */
export function useChatSocket() {
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      if (closedByEffect) return;
      // No stored access token means the user is logged out (or logging out) — retrying
...
function handleEvent(qc: QueryClient, event: ChatEvent) {
  ...
  if (event.type === 'RECEIPT_UPDATED') {
    ...
    // right back — a self-sustaining ping-pong between two open clients. This handler simply
    // never triggers a network call in the first place.
