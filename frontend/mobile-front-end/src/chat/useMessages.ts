...
import { fetchUserBySub } from '@/src/api/usersApi';
import type { CreateConversationRequest, PresenceStatus, SendMessageRequest } from '@/src/api/chatTypes';

// Live updates arrive via useChatSocket (mounted at the app root); this is just a
// safety-net poll in case a WS event is missed or the socket is momentarily disconnected.
...
// thread and friends. This mirrors a real bug hit on web: an unhandled WS event type
// invalidating just the list also refetched the open thread, and that refetch's side effects
// re-emitted the same event, looping forever.
 ...
 * toggle for everyone, including the actor, the same way the rest of this hook set works. */
