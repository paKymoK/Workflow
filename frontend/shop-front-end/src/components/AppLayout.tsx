...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchHealth = async () => {
      try {
        const { data } = await api.get("/api/health?monitors=auth-service,shop-service");
  ...
  const serviceListHint = services.length > 4 ? `+${services.length - 4} more` : "";
  ...
  const siderMenuItems = [
    { key: "/", icon: <HomeOutlined />, label: "Shop" },
  ];
      ...
      <Layout className="h-screen">
        ...
        >
          ...
          <Menu
            theme={isDark ? "dark" : "light"}
            mode="inline"
            selectedKeys={[location.pathname === "/" || location.pathname.startsWith("/") ? "/" : "/"]}
